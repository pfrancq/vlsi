...
/**	KDevVLSIDoc provides a document object for a document-view model.
  *
  * The KDevVLSIDoc class provides a document object that can be used in conjunction with the classes
  * KDevVLSIApp and KDevVLSIView to create a document-view model for MDI (Multiple Document Interface)
  * KDE 2 applications based on KApplication and KTMainWindow as main classes and QWorkspace as MDI manager widget.
  * Thereby, the document object is created by the KDevVLSIApp instance (and kept in a document list) and contains
  * the document structure with the according methods for manipulating the document
  * data by KDevVLSIView objects. Also, KDevVLSIDoc contains the methods for serialization of the document data
  * from and to files.
...
class KDevVLSIDoc : public QObject
{
  ...
  public:
    ...
    /** returns the first view instance */
		KDevVLSIView* firstView(){ return pViewList->first(); };
